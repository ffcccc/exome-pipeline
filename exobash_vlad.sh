...
if $MAP2Bam; then
	...
	if true; then
	~/bin/bwa/bwa aln -t ${NCORES} ${RefGENOME} ${MAP1}.fq > ${MAP1}.sai
	~/bin/bwa/bwa aln -t ${NCORES} ${RefGENOME} ${MAP2}.fq > ${MAP2}.sai
#2 Convert SAI to SAM (BWA): r1.sai, r2.sai ==> .sam 
	~/bin/bwa/bwa sampe -P ${RefGENOME} ${MAP1}.sai ${MAP2}.sai ${MAP1}.fq ${MAP2}.fq > ${MAPPED}.sam
 	fi
...
if $INDELRealign; then
	...
	#  performance 500MB bam/~7000000 reads: 1 core a-> 3hh 
